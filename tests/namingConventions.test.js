...
describe('Naming conventions', () => {
    const variables = JSON.parse(fs.readFileSync('/tmp/variables.json', {encoding: 'utf-8'}));
    ...
    it('starts with a valid department', () => {
        ...
        managementGroupSubscriptions
            .filter(sub => !globalExclusions.includes(sub) && !localExclusions.includes(sub))
    ...
    it('ends with a valid environment', () => {
        const localExclusions = ['DTS-Terraform-Dev-Test4', 'DTS-RBAC-NONPRODUCTION']

        managementGroupSubscriptions
            .filter(sub => !globalExclusions.includes(sub) && !localExclusions.includes(sub))
            .forEach(subscription => {
                expect(subscription).toMatch(/^(.+)-(.+)-(SBOX|DEV|TEST|ITHC|DEMO|STG|PROD)/)
            })
    })
