resource "azurerm_role_assignment" "custom_role_assignments" {
  ...
  role_definition_id = var.create_custom_roles == true ? "/subscriptions/${var.subscription_id}${each.value.role_definition_resource_id}" : "/subscriptions/${var.subscription_id}${each.value.id}"
