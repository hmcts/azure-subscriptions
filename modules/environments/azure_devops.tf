...
resource "azuredevops_serviceendpoint_azurerm" "ptl_endpoint" {
  ...
  azurerm_spn_tenantid      = data.azurerm_client_config.current.tenant_id
  azurerm_subscription_id   = data.azurerm_subscriptions.sds_ptl[0].subscriptions[0].subscription_id
  azurerm_subscription_name = data.azurerm_subscriptions.sds_ptl[0].subscriptions[0].display_name
}
